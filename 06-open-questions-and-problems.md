...
3.  Check-in, proof, disputes, substitutions, forfeits, announcements, messaging, print/share, and broader admin overrides beyond completed-match corrections are later scope unless explicitly moved. Direct participant score reporting, constrained admin correction of completed match results, controlled late participant placement, and tournament cancellation are MVP. Tournament reopening is not supported.
...
5.  Completion requires every required match and round to be complete and standings and placements to calculate successfully. Completed is final and has no outgoing transition; only admin edits to participant faction and faction-specific rules text, and constrained completed-match result corrections, remain allowed.
...
4.  Assigned participants cannot change completed matches. An admin may correct a completed result through the constrained correction flow. `Ready`, `Ready for Score Reporting`, `In Progress`, `Awaiting Score`, `Score Submitted`, `Under Review`, `Disputed`, and `Forfeited` are not MVP match states.

Before start, affected setup changes automatically regenerate the current admin-only preview without retaining preview history. At start, format, structure, scoring, standings points, tiebreakers, seeds, and bracket or schedule rules lock. Description, rules summary, and the tournament scheduled date/time remain editable during in-progress play. A completed-match correction is the sole allowed change to a completed result and may reflow only unfinished competition.
...
### Resolved Tabletop Wargames domain behavior

1.  Game system or activity remains required tournament information. Edition, points limit, and army size are not separate MVP fields; an admin may cover them in the rules summary.
2.  Faction and faction-specific rules text are optional per-participant metadata managed by an admin. They may be edited in every state except cancelled, including after completion, and do not alter competition results or history.
3.  MVP has no army or list submission. Faction-specific rules text is not a list submission field.
4.  Table assignment, missions, scenarios, deployment maps, round packets, per-round scheduled start/end times, timers, and late-arrival handling are later scope.

### Resolved completed-match correction behavior

1.  In every non-cancelled tournament state, including completed, an admin may submit a valid replacement result for a completed match after reviewing and confirming its impact.
2.  The correction updates standings and final placements and reflows only affected not-ready or active competition.
3.  The system blocks a correction that would change the entrant, opponent, or pairing of a completed downstream match. In single elimination, a downstream match receives an entrant from the corrected match. In Swiss, it includes a completed later-round pairing that would differ after recalculation. Round-robin schedules are fixed, so corrections recalculate standings only.

### Participant and registration behavior

1.  Define whether admin approval, rejection, waitlists, registration limits, and registration deadlines are MVP or later scope beyond the basic MVP joining controls.
2.  Define whether participants need accounts in MVP or can join with lightweight public information.
3.  Define how duplicate participant names are detected or handled.
4.  Define the lifecycle and visibility differences between an admin-removed participant and a participant who withdrew, including whether either can rejoin and how either appears in previews and public lists.
5.  Define how an admin-added or bulk-added offline participant is associated with a later participant identity, and who may withdraw that entry.
6.  Define the bulk participant input format, validation and normalization rules, duplicate handling, error feedback, and whether a partly invalid import is applied or rejected.
...
4.  Define the guardrails for broader future admin overrides beyond the MVP completed-match correction, which must not alter completed tournament history without explicit confirmation.
